    ...
    <>
      ...
      <div className="app-shell">
        ...
        <main className="main-content">
          ...
          {view === "users" && <Users roles={roles} create={(user) => { if (users.some((item) => item.email.toLowerCase() === user.email.toLowerCase())) return; setUsers((all) => [user, ...all]); emitRegisteredHooks("user.created", { user }); notify("User created"); }} update={(next) => { setUsers((all) => all.map((user) => user.id === next.id ? next : user)); emitRegisteredHooks("user.updated", { user: next }); notify("User updated"); }} remove={(user) => { if (!window.confirm(`Delete user ${user.name}?`)) return; setUsers((all) => all.filter((item) => item.id !== user.id)); setEntries((all) => all.map((entry) => entry.authorUserId === user.id ? { ...entry, authorUserId: undefined } : entry)); emitRegisteredHooks("user.deleted", { userId: user.id }); notify("User deleted"); }} />}{" "}           {view === "roles" && <Roles roles={roles} create={(role) => { if (roles.some((item) => item.slug === role.slug)) return; setRoles((all) => [role, ...all]); emitRegisteredHooks("role.created", { role }); notify("Role created"); }} />}
...
function Roles({ roles, create }: { roles: Role[]; create: (role: Role) => void }) {
