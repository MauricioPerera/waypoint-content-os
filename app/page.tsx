...
function PageBuilder({
  ...
  return <div className="page">
    ...
    <section className="card visual-preview-card" style={{ marginTop: 16 }}><div className="card-heading"><div><p className="eyebrow">LIVE PREVIEW</p><h2>Responsive canvas</h2></div><div className="preview-device-pills"><span>Desktop</span><span>Tablet</span><span>Mobile</span></div></div><div className="visual-preview" style={{ maxWidth: layoutDraft.rules.maxWidth }}>{page?.blocks.length ? page.blocks.map((block) => <div key={block.id} className="visual-preview-block">{previewBlock(block)}</div>) : <p className="subhead">Add a block to see the page preview.</p>}</div></section>
