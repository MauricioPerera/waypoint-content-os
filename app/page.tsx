...
function Plugins({
  ...
  const [testRuns, setTestRuns] = useState<Array<{ id: string; kind: "hook" | "action"; label: string; createdAt: string }>>([]);
  ...
  return (
    <div className="page">
      ...
      {testRuns.length > 0 && <section className="card entries-card" style={{ marginTop: 16 }}><div className="card-heading"><div><p className="eyebrow">TEST HISTORY</p><h2>Recent extension runs</h2></div><span className="live-chip"><i />{testRuns.length} runs</span></div><div className="activity-list">{testRuns.map((run) => <Activity key={run.id} icon={run.kind === "hook" ? "↗" : "✦"} title={`${run.kind} test · ${run.label}`} detail="Safe local dispatch" time={new Date(run.createdAt).toLocaleString()} />)}</div></section>}
